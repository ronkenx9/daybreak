...
assert.throws(()=>p.validatePaperTradePrecision(p.quotePaperTrade(original,'buy',1e-11)),/below simulation precision/);
p.validatePaperTradePrecision(p.quotePaperTrade(original,'buy',1));
const pending=loader()('lib/theses/paper-pending.ts');
const values=new Map(),storage={getItem:key=>values.get(key)??null,setItem:(key,value)=>values.set(key,value),removeItem:key=>values.delete(key)};
const creation={version:1,accountId:'account-a',intentId:randomUUID(),createdAt:Date.now(),input:{instrumentId:'stock:aapl',title:'Services growth',summary:'A durable test thesis',tokenName:'Apple thesis',tokenSymbol:'IDEA'}};
pending.writePendingPaperCreation(storage,creation);
assert.deepEqual(pending.readPendingPaperCreation(storage,'account-a'),creation);
assert.throws(()=>pending.writePendingPaperCreation(storage,{...creation,intentId:randomUUID()}),/CONFLICT/);
pending.clearPendingPaperCreation(storage,{accountId:creation.accountId,intentId:'different'});assert(pending.readPendingPaperCreation(storage,'account-a'));
pending.clearPendingPaperCreation(storage,creation);assert.equal(pending.readPendingPaperCreation(storage,'account-a'),null);
