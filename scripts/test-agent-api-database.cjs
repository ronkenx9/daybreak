...
(async()=>{
 ...
 try{
  ...
  const first=await repo.createAgentForOwner(owner,input('Catalyst One')),second=await repo.createAgentForOwner(owner,input('Catalyst Two'));
  ...
  const profile=await repo.getPublicAgentProfile(two.publicId);assert.equal(profile.name,'Catalyst Two');assert(!JSON.stringify(profile).includes(owner));assert(!JSON.stringify(profile).includes('Private Operator'));
