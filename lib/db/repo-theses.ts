...
import { operations, profiles, theses, thesisMarkets, thesisTradeQuotes } from './schema';
...
export async function listPublishedTheses(limit = 40) {
  ...
  return db.select({
    ...
    tokenSymbol: theses.tokenSymbol, status: theses.status, publishedAt: theses.publishedAt,
    ...
    .innerJoin(thesisMarkets, eq(thesisMarkets.thesisId, theses.id))
    .leftJoin(profiles, eq(profiles.userId, theses.authorUserId))
    .where(and(eq(theses.visibility, 'public'), inArray(theses.status, ['published', 'withdrawn']), inArray(thesisMarkets.status, ['active', 'migrated'])))
...
export async function getPublicThesis(idOrSlug: string) {
  ...
  const rows = await db.select({
    ...
    .where(and(
      ...
      inArray(thesisMarkets.status, ['active', 'migrated']),
    )).limit(1);
  return rows[0] ?? null;
}
