...
import { PAPER_SETTLEMENT_UNIT } from '@/lib/theses/paper';
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!Number.isFinite(amount) || amount < PAPER_SETTLEMENT_UNIT || amount > 1_000_000) throw new HttpError(400, 'Enter a valid paper amount');
    ...
    if (error instanceof Error && error.message === 'PAPER_ACTIVITY_LIMIT') return errorResponse(new HttpError(429, 'Too many paper trades. Try again in a minute.'));
    if (error instanceof Error && error.message.startsWith('Paper amount is below simulation precision')) return errorResponse(new HttpError(400, error.message));
