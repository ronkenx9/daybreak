...
export async function POST(req:Request,ctx:Context){let running:{userId:string;id:string}|undefined;try{
 ...
 if(path[0]==='publish'&&uuid(body.id)){if(!await store.publishJob(user.id,body.id))throw new HttpError(404,'Finished creation not found');return Response.json({imageUrl:`https://www.daybreakcircles.lol/api/muse/art/${body.id}`});}
  ...
  if(path[0]==='token-brief'){if(typeof body.ticker!=='string'||!TOKENS.some(t=>t.ticker===body.ticker)||typeof body.moment!=='string'||!body.moment.trim()||body.moment.length>800)throw new HttpError(400,'Need a stock and a moment for token details');const company=TOKENS.find(t=>t.ticker===body.ticker)?.name??body.ticker;const brief=await museRequest(user.id,{action:'token-brief',ticker:body.ticker,company,moment:body.moment.slice(0,800)});const d=brief.data as {name?:unknown;symbol?:unknown;description?:unknown};const name=String(d.name??'').slice(0,40);const symbol=String(d.symbol??'').toUpperCase().replace(/[^A-Z0-9]/g,'').slice(0,10);const description=String(d.description??'').slice(0,140);if(name.length<2||symbol.length<2||!description)throw new HttpError(502,'The details did not land. Try again.');return Response.json({name,symbol,description});}
