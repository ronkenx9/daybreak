...
export async function GET(){
 ...
 try{const data=await cached('prices',readPrices);if(!Object.values(data).some(p=>p.state!=='unavailable'))throw Error('No price data');return NextResponse.json(Object.fromEntries(Object.entries(data).map(([k,v])=>[k,priceState(v)])),{headers:{'Cache-Control':'public, s-maxage=30, stale-while-revalidate=300'}})}
