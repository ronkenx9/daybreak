...
- Flash's documented shared key worked for the earlier read-only quote, but Daybreak's own integrator key has not been configured. The application no longer falls back to the shared key: stock-order controls are unavailable, and Flash API requests return 503 until `DEFINITIVE_FLASH_API_KEY` is set server-side. No live order has been submitted.
