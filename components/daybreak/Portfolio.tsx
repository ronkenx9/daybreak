...
export default function Portfolio({snapshot,error,loading,refresh,refreshing}:{snapshot?:HoldingsSnapshot;error:boolean;loading:boolean;refresh:()=>void;refreshing:boolean}){
 ...
 {snapshot&&<><div className="db-portfolio-value"><span>{complete?'Reference value':'Priced subtotal'}{error||tooOld?' · previous snapshot':''}</span><h2 className="db-shine">{totals&&((snapshot.holdings.length===0&&complete)||totals.priced>0)?usd(totals.total):'Unavailable'}</h2></div>
 ...
 {snapshot.holdings.length>0?<div className="db-holdings-list">{snapshot.holdings.map(h=>{const p=snapshot.prices[h.ticker],v=positionUsdAtoms(h,p);return <div className="db-holding-row" key={h.token}><StockIcon ticker={h.ticker} size={44}/><div className="db-holding-main"><strong>{h.name}</strong><small>{h.shares} underlying shares</small><small>{h.tokenQuantity} {h.onchainSymbol} tokens</small></div><div className="db-holding-val"><strong className="db-shine">{v===null?'Unavailable':usd(v)}</strong><small>{p?.state==='paused'?'Oracle paused':p?.updatedAt?`Oracle as of ${new Date(p.updatedAt).toLocaleString()}`:'No price'}</small></div></div>})}</div>:snapshot.status==='complete'?<p>No supported tokenized stocks found at this block.</p>:<p>No positive balances found among the tokens successfully checked.</p>}</>}
 </section>;
}
