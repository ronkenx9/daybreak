...
type Challenge = { id: string; title: string; brief: string; criteria: string; budgetCents: number; deadline: string; status: string; isSponsor: boolean; canDispute: boolean };
...
export default function CircleEconomy({ slug, canFund }: { slug: string; canFund: boolean }) {
  ...
  const dispute = async (event: React.FormEvent, id: string) => {
    event.preventDefault(); setBusy(true); setError(''); setNotice('');
    try { await authedFetch(`/api/economy/challenges/${id}/dispute`, { method: 'POST', body: JSON.stringify({ reason: disputeReason }) }); setDisputing(''); setDisputeReason(''); setReviewing(''); setNotice('Challenge paused for review. Its funded credits remain reserved.'); await refresh().catch(() => setNotice('Dispute opened. Reload this Circle to refresh its status.')); }
    catch (caught) { setError(caught instanceof Error ? caught.message : 'Could not open a dispute.'); }
    finally { setBusy(false); }
  };
