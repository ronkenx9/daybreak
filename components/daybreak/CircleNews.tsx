...
export default function CircleNews({ slug, isMember, title }: { slug: string; isMember: boolean; title?: string }) {
  ...
  return <div className="db-create-news">
    ...
    : items.length > 0 ? <div className="db-create-news-row">{items.map((n) => <button type="button" key={`${n.ticker}-${n.url}`} onClick={() => setOpenUrl(n.url)}><strong>{n.ticker}</strong><span>{n.title}</span><small><MessageCircle size={12}/> Discuss</small></button>)}</div>
