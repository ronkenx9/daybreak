...
export default function Memestocks({ticker}:{ticker:string}){
 ...
 return <section className="db-memestocks"><div className="db-section-heading"><h3>Community memestocks</h3><div className="db-meme-actions"><Link className="db-text-link" href={`/app/create?stock=${ticker}`}>Create artwork</Link><Link className="db-text-link" href={`/app/conviction?stock=${ticker}`}><Lightbulb size={15}/> Create thesis</Link><button className="db-text-link" disabled={q.isFetching} onClick={()=>void q.refetch()}>{q.isFetching?'Loading…':'Refresh'}</button></div></div>
