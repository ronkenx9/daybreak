...
export default function Dialog({label,onClose,children}:{label:string;onClose:()=>void;children:ReactNode}){
 ...
 return <div className="db-dialog-overlay" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}><div ref={ref} tabIndex={-1} role="dialog" aria-modal="true" aria-label={label} className="db-company-dialog">{children}</div></div>;
}
