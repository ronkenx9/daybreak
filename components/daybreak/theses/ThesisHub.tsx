...
export default function ThesisHub() {
  ...
  useEffect(()=>{const controller=new AbortController();setState('loading');const params=new URLSearchParams({mode:tab,q:query,page:String(page)});const timer=setTimeout(()=>{fetch(`/api/theses?${params}`,{signal:controller.signal}).then(r=>{if(!r.ok)throw Error();return r.json()}).then(data=>{setItems(data.items||[]);setHasMore(!!data.hasMore);setState('ready')}).catch(error=>{if(error?.name!=='AbortError')setState('error')})},200);return()=>{clearTimeout(timer);controller.abort()}},[tab,query,page,refresh]);
