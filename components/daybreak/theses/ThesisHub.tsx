...
export default function ThesisHub() {
  ...
  useEffect(()=>{const controller=new AbortController();setState('loading');const params=new URLSearchParams({mode:tab,q:query,page:String(page)});const timer=setTimeout(()=>{fetch(`/api/theses?${params}`,{signal:controller.signal}).then(r=>{if(!r.ok)throw Error();return r.json()}).then(data=>{setItems(data.items||[]);setHasMore(!!data.hasMore);setState('ready')}).catch(error=>{if(error?.name!=='AbortError')setState('error')})},200);return()=>{clearTimeout(timer);controller.abort()}},[tab,query,page]);
  ...
  if(mode==='paper')return <PaperTradingMode instruments={instruments} instrumentState={instrumentState} initialThesis={paperSeed} onPublished={(thesis)=>{setItems(current=>[thesis,...current.filter(item=>item.id!==thesis.id)]);setPaperSeed(thesis)}} onClose={()=>{setPaperSeed(null);setMode('feed')}}/>;
  ...
  return <section className="db-thesis-hub">
    ...
    <div className="db-thesis-toolbar"><label><Search size={16}/><input value={query} onChange={event=>{setQuery(event.target.value);setPage(0)}} placeholder="Search theses or companies" aria-label="Search theses or companies"/></label><div className="db-thesis-tabs">{([['all','All'],['paper','Paper'],['live','Live']] as const).map(([id,label])=><button key={id} aria-pressed={tab===id} onClick={()=>{setTab(id);setPage(0)}}>{label}</button>)}</div></div>
