...
export default function PublicThesisPage({ thesis, instrument }: { thesis: ThesisView; instrument?: ThesisInstrumentView }) {
  return <main className="db-public-thesis">
    <header><Link href="/"><Wordmark/></Link><Link href={`/app/conviction?thesis=${thesis.slug}`} className="db-button db-blue-button">Open in Daybreak <ArrowRight size={15}/></Link></header>
    ...
    <div className="db-public-thesis-grid"><article>
      <div className="db-thesis-company"><StockIcon ticker={instrument?.ticker||'AAPL'} size={48}/><div><strong>{instrument?.companyName||thesis.companyId}</strong><span>{thesis.tokenSymbol} / {instrument?.symbol||'stock token'} · Solana</span></div><span className="db-thesis-stage">{thesis.marketStatus}</span></div>
    ...
    </article><aside className="db-thesis-public-cta"><span className="db-eyebrow">Daybreak thesis market</span><h2>Read first. Back second.</h2><p>Inspect the exact pair and current quote inside Daybreak before signing anything.</p><Link href={`/app/conviction?thesis=${thesis.slug}`} className="db-button db-blue-button">Review this market <ArrowRight size={16}/></Link><small>Backing exchanges the stock token for a thesis position. It is not equity or a guaranteed return.</small></aside></div>
  </main>;
}
