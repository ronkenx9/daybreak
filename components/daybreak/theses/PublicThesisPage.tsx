...
import ThesisBacking from './ThesisBacking';
...
export default function PublicThesisPage({ thesis, instrument }: { thesis: ThesisView; instrument?: ThesisInstrumentView }) {
  return <main className="db-public-thesis">
    <header><Wordmark/><Link href={`/app/conviction?thesis=${thesis.slug}${thesis.mode==='paper'?'&simulate=1':''}`} className="db-button db-blue-button">Open in Daybreak <ArrowRight size={15}/></Link></header>
    ...
    <div className="db-public-thesis-grid"><article>
      ...
      <h1>{thesis.title}</h1><XLayerBackingBadge slug={thesis.slug} companyId={thesis.companyId}/><p className="db-thesis-lede">{thesis.summary}</p>
