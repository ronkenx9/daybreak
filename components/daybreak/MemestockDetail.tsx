...
export interface MemeTokenData{symbol:string;name:string;address:string;priceUsd:number|null;liquidityUsd:number;volume24Usd:number;url:string;lowLiquidity:boolean;volume?:{h1:number;h6:number;h24:number};change?:{h1:number;h6:number;h24:number};marketCapUsd?:number|null;txns24?:number;ageMs?:number|null}
...
export default function MemestockDetail({token,pairedWith,companyName}:{token:MemeTokenData;pairedWith:string;companyName:string}){
 return <div className="db-meme-detail">
  <div className="db-meme-detail-head"><span className="db-token-mono db-meme-mono">{token.symbol.slice(0,2)}</span><div><h3>{token.name||token.symbol}</h3><span className="db-ticker">{token.symbol} · paired with {pairedWith}</span></div><TypeBadge kind="meme"/></div>
