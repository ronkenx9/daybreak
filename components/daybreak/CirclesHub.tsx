...
export default function CirclesHub() {
  ...
  const requestedContext = useMemo(() => {
    if (typeof window === 'undefined') return { circle: '', story: '' };
    const params = new URLSearchParams(window.location.search);
    return { circle: params.get('circle') ?? '', story: params.get('story') ?? '' };
  }, []);
  ...
  const load = async (prefer?: string) => {
    ...
    try {
      ...
      setActiveSlug((current) => prefer ?? (result.circles.some((circle) => circle.slug === requestedContext.circle) ? requestedContext.circle : current || result.circles[0]?.slug || ''));
  ...
  const syncSolana = async () => {
    ...
    try {
      ...
      setNotice(result.eligibility.tickers.length ? `Unlocked from verified Solana holdings: ${result.eligibility.tickers.join(', ')}.` : 'Solana wallet verified. No supported xStock or PreStocks balances were found.');
      await load();
    } catch (error) {
